...
function mapRoundLog(entry: ApiRecord): PublicGoodsRoundLog {
    return {
        round: resolveProp<number>(entry, "round") ?? 1,
        actions: (entry.actions as Record<string, string>) ?? {},
        numericContributions:
            (resolveProp<Record<string, number>>(
                entry,
                "numericContributions",
                "numeric_contributions"
            ) ?? {}) as Record<string, number>,
        totalPool:
            resolveProp<number>(entry, "totalPool", "total_pool") ?? 0,
        payoffs: (entry.payoffs as Record<string, number>) ?? {},
    };
}

export async function createPublicGoodsGame(
    hostId: string,
    communicationMode: "communication" | "no-communication",
    rounds: number
): Promise<CreateGameResponse> {
    const response = await fetch(`${API_BASE}/games/public-goods/create`, {
        method: "POST",
    ...
    if (!response.ok) {
        ...
        throw new Error(error.detail || "Failed to create Public Goods Game");
    }
    return response.json();
}

export async function getPublicGoodsSession(
    sessionId: string
): Promise<PublicGoodsSessionState> {
    const response = await fetch(
        `${API_BASE}/games/public-goods/sessions/${sessionId}`
    );
    if (!response.ok) {
        const error = await response.json().catch(() => ({}));
    ...
    return {
        ...
        roundLogs: roundLogsRaw.map(mapRoundLog),
...
export async function submitPublicGoodsAction(
    sessionId: string,
    participantId: string,
    action: PublicGoodsAction
): Promise<void> {
    const response = await fetch(
        `${API_BASE}/games/public-goods/sessions/${sessionId}/actions?participant_id=${encodeURIComponent(
            participantId
        )}`,
        {
            method: "POST",
...
export async function leavePublicGoodsGame(
    sessionId: string,
    participantId: string
): Promise<void> {
    await fetch(
        `${API_BASE}/games/public-goods/sessions/${sessionId}?participant_id=${encodeURIComponent(
            participantId
        )}`,
        { method: "DELETE" }
    );
