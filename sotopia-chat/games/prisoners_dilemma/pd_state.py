"""Redis state helpers for Prisoner's Dilemma sessions."""
...
STATE_PREFIX = "prisoners_dilemma:session"
...
class PDStateStore:
    """Async Redis store for PD state + queued actions."""
    ...
    async def write_state(
        ...
        *,
        ...
        client = self._client()
        try:
            await client.set(state_key(session_id), json.dumps(payload), ex=ttl)
        finally:
            await client.aclose()
